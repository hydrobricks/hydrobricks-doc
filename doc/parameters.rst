...
.. note::
   ...
   The canonical single-objective workflow described here can also be run
   declaratively, without this boilerplate: a ``calibration`` section in a
   :ref:`project file <project-files-calibration>` (one call,
   ``project.calibrate()``), or a :ref:`study file <study-files>` to compare
   several settings (catchments, models, objectives, transformations, ...) in
   one experiment. The trainer API below remains the way to customize beyond
   that (auxiliary observations, Pareto, parallel backends, ...).
...
.. note::
   ...
   ``obj_func`` accepts any `HydroErr
   <https://hydroerr.readthedocs.io/en/stable/list_of_metrics.html>`_ metric name, plus
   ``'kge_np'`` (alias ``'kge_non_parametric'``) for the non-parametric Kling-Gupta
   Efficiency (:cite:t:`Pool2018`). When ``obj_func`` is omitted, the non-parametric KGE
   is used by default. It is computed via the optional SPOTPY dependency.

A ``transform`` argument shifts the discharge objective's emphasis across the flow
regime — e.g. towards low flows, using a power or log transform (see
:ref:`discharge transformations <discharge-transformations>`):
...
It applies to the discharge objective only (never to the auxiliary
``extra_observations`` signals), including when it is combined and normalized
with those signals.
...
``evaluate_periods()`` also accepts a ``transform`` (see :ref:`discharge
transformations <discharge-transformations>`), applied to each period's observed
and simulated series before computing its metrics; with an ``'auto'`` epsilon it
is resolved separately per period, from that period's own observations. A
**list** of transforms evaluates them all at once — the columns then become a
``(transform, metric)`` MultiIndex:
...
.. code-block:: python
   ...
   scores['power(0.2)']   # the low-flow-emphasis columns
...
.. _glacier_mass_balance_by_type:

Seasonal balances as separate signals
"""""""""""""""""""""""""""""""""""""

``from_glamos`` with several ``balance_types`` **pools** them: the annual, winter and
summer values are concatenated into a single vector scored by a single metric. When the
file reports the three, that is rarely what you want. The benchmark an error metric is
normalized against (see *Comparable ranges* below) is then the spread of the pooled
vector, which is dominated by the winter/summer contrast — their means have opposite
signs and are hundreds to thousands of mm w.e. apart — rather than by the interannual
variability the melt parameters control. A model that merely reproduces the seasonal
cycle already scores well, while possibly being *worse* than the per-season climatology
on each season taken alone.

:meth:`from_glamos_by_type
<hydrobricks.evaluation.glacier_mass_balance.GlacierMassBalanceObservations.from_glamos_by_type>`
loads the same file as **one signal per balance type** instead, returning a list ready to
be passed as ``extra_observations``:
...
.. code-block:: python
   ...
   glacier_mb = hb.GlacierMassBalanceObservations.from_glamos_by_type(
       ...
       glacier_id='B43-03',
       metric='rmse',
       total_weight=0.5,   # weight of the mass balance as a whole
       weight_shares={'annual': 2.0, 'winter': 1.0, 'summer': 1.0},
   )

Each signal is then scored against **its own** benchmark, carries its own ``weight`` and
— in ``'constraint'`` mode — its own tolerance, relative to its own observed mean.
``total_weight`` is distributed over the returned signals following ``weight_shares``;
only the ratios matter, so the example above gives the annual balance twice the weight of
each season. Every signal knows its ``balance_type``, which labels it in reports and
plots:

.. code-block:: python

   for signal in glacier_mb:
       print(signal.balance_type, len(signal), signal.weight)
   # annual 11 0.25
   # winter 11 0.125
   # summer 12 0.125

Giving the annual balance a larger share is usually the right default: it is the quantity
the glacier's fate depends on, and since it is (to the fixed-date bookkeeping) the sum of
the winter and summer balances, an even split lets the optimizer buy two seasonal
improvements with one worse annual value.

**Missing balance types.** Seasonal balances are not always reported: many series (WGMS
entries, geodetic surveys, most non-Swiss monitoring) carry the annual balance only, and
GLAMOS itself leaves the winter date and the ``Bw`` / ``Bs`` columns empty for some
glaciers and years. A requested type with no usable row is **skipped** (with a log
message) rather than returned as an empty signal, and ``weight_shares`` is renormalized
over the types actually found — so an annual-only source gets the whole
``total_weight`` instead of a third of it. A ``DataError`` is raised if none of the
requested types has a usable observation.

.. note::

   An empty auxiliary signal cannot be scored, and :class:`SpotpySetup
   <hydrobricks.trainer.SpotpySetup>` rejects one at construction with a ``DataError``
   naming it (each signal exposes a ``name``, e.g.
   ``glacier mass balance (winter, whole)``). Left in place it would make every parameter
   set score the rejection penalty, which looks like a calibration that never finds a
   behavioural run.

Use plain ``from_glamos`` when a single, pooled signal is what you want — with a
single balance type the two are equivalent. ``from_csv`` already loads one balance type
per call, so build the list yourself (and set each ``weight``) for a non-GLAMOS source
that reports several.
...
* ``mode='objective'`` with ``combine='pareto'`` — returns a
  ``[discharge, *objective signals]`` objective vector for a multi-objective sampler
  (SPOTPY's ``NSGAII``), yielding a Pareto front rather than a single best run. The
  vector has one component per ``'objective'`` signal plus the discharge; read its length
  from ``SpotpySetup.n_objectives`` rather than counting by hand, and pass it as the
  sampler's ``n_obj`` so the two always agree.
...
.. note::
   ...
   **benchmark skill score**: an error metric ``E`` becomes ``1 - E / E_ref`` where
   ...
   instead. With ``combine='pareto'`` each component is oriented (and normalized) the
   same way, but they are neither weighted nor summed — the sampler optimizes them
   separately, which is the point of a Pareto front.
...
* ``mode='constraint'`` — a behavioural pass/fail filter: runs whose mean absolute
  ...
  absolute observed value, e.g. ``0.1`` for 10 %). Set exactly one of the two. With one
  signal per balance type a run must pass every filter, and a relative tolerance is taken
  against each type's own observed mean instead of the pooled one. Weights play no role
  here: constraints filter, they do not score.
...
.. code-block:: python
   ...
   sampler = trainer.calibrate(
       ...
       sample_kwargs={'n_obj': spot_setup.n_objectives, 'n_pop': 50},
   )
...
with the observation's ``simulated`` method after a run — once per signal when the
balance types are split:

.. code-block:: python

   socont.run(parameters=parameters, forcing=forcing)
   for signal in glacier_mb:
       sim_mb = signal.simulated(socont)      # aligned with signal.observed()
       print(signal.balance_type, hb.evaluate(sim_mb, signal.observed(), 'rmse'))
